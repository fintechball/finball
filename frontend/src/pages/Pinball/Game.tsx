...
function App() {
  ...
  function start() {
    for (let i = 0; i < totalCnt; i++) {
      const ball = Bodies.circle(X[Math.floor(Math.random() * X.length)], Y[Math.floor(Math.random() * Y.length)], width / 55, {
        restitution: 1,
        friction: 0.1,
        density: 5,
        isStatic: false,
        render: {
          ...
          sprite: {
            ...
            // texture: '',
            xScale: Math.sqrt(width ** 2 + height ** 2) / 23 / 250,
            yScale: Math.sqrt(width ** 2 + height ** 2) / 23 / 250,
          },
        },
      });
      balls.push(ball);
